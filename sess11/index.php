...
if ($contacts_query) {
    ...
    <table border="1">
        <?php while ($row = mysqli_fetch_array($contacts_query)) {
            $strListName = array();
            $assignedListQuery = mysqli_query($connection,"SELECT * FROM contacts_list_assigned assi JOIN contact_lists lists ON lists.list_id = assi.list_id WHERE assi.cnt_id = ".$row['cnt_id']);

            while ($listRow = mysqli_fetch_array($assignedListQuery)) {
                $strListName[] = $listRow["list_name"];
            }
